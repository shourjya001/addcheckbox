...
AS $function$
DECLARE
    OpenedTran BOOLEAN DEFAULT false;
    Error INTEGER;
    CODFILE INTEGER;
    CODRIA INTEGER;
    CRIA INTEGER;
    ...
    SWV_error INTEGER := 0;
BEGIN
    -- Validate status and get exchange rate
    IF "PS_CODSTATUS" NOT IN (25, 26) THEN
        RAISE EXCEPTION 'Invalid credit file status';
    END IF;
    ...
    -- Store historical data before update
    ...
    SELECT 
        ...
        LINE."CODSPM",
        RIA."CODSTATUS",
        COALESCE(LINE."TB01",0), COALESCE(LINE."TB02",0),
        COALESCE(LINE."TB03",0), COALESCE(LINE."TB04",0),
        COALESCE(LINE."TB05",0), COALESCE(LINE."TB06",0),
        COALESCE(LINE."TB07",0), COALESCE(LINE."TB08",0),
        COALESCE(LINE."TB09",0), COALESCE(LINE."TB10",0),
        COALESCE(LINE."TB11",0), COALESCE(LINE."TB12",0),
        COALESCE(LINE."TB13",0), COALESCE(LINE."TB14",0),
        LINE."CODTRC",
        NOW(),
        "PS_CODUSR"
    FROM "dbo"."TCDTFILEDBE" CDT
    JOIN "dbo"."TRIADBE" RIA ON CDT."CODFILE" = RIA."CODFILE"
    JOIN "dbo"."TOCDBE" OC ON RIA."CODOC" = OC."CODOC"
    JOIN "dbo"."TLINERIADBE" LINE ON RIA."CODRIA" = LINE."CODRIA"
    ...
    -- Process Credit File on Creation (25)
    ELSIF "PS_CODSTATUS" = 25 THEN
        UPDATE "dbo"."TLINERIADBE"
        SET
            "TB01" = CAST(("TB01"/RATE) AS NUMERIC(7,2)),
            "TB02" = CAST(("TB02"/RATE) AS NUMERIC(7,2)),
